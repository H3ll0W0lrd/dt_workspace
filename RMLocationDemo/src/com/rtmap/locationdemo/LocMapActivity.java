...
public class LocMapActivity extends Activity implements RMLocationListener,
	...
	private void initMap() {
		...
		Bitmap black = RMIOUtils.drawableToBitmap(getResources().getDrawable(
				R.drawable.sign_black));
		Bitmap gray = RMIOUtils.drawableToBitmap(getResources().getDrawable(
				R.drawable.sign_gray));
		Bitmap green = RMIOUtils.drawableToBitmap(getResources().getDrawable(
				R.drawable.sign_green));
		String[] colorarray = new String[] { "black", "gray", "green" };
//		BeanLayer beanLayer = new BeanLayer(mMapView, bitmap);
//		beanLayer.addBeanIcon(colorarray[0], black);
//		beanLayer.addBeanIcon(colorarray[1], gray);
//		beanLayer.addBeanIcon(colorarray[2], green);
//		beanLayer.setRange(15f);
//		int x = 300, y = 100;
//		for (int i = 0; i < 10; i++) {
//			y += 3;
//			x = 2;
//			for (int j = 0; j < 10; j++) {
//				Bean b = new Bean(i, "860100010040300001", "F3", x += 5, y,
//						colorarray[(int) (Math.random() * 3)]);
//				beanLayer.addBean(b);
//			}
//		}

//		mMapView.addLayer(beanLayer);
	...
	private void initLocation() {
		...
		LocationApp.getInstance().setPersistentMotionEnable();// 开启检测传感器参数
	}
	...
	public void onReceiveLocation(RMLocation result) {
		mText.setText(result.getError() + "  " + result.getBuildID() + "   "
				+ result.getFloor() + "   精度：" + result.accuracy + "米\nx: "
				+ String.format("%.3f", result.getX()) + "   y: "
				+ result.getY() + "\nUid：" + result.getUserID() + "\n推算类型："
				+ result.getCalculateType() + "\n偏转角："
