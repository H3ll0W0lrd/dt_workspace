...
public class KPHttpClient {
	...
	public static boolean checkConnection() {
		ConnectivityManager connectivityManager = (ConnectivityManager) KPApplication
				.getInstance().getSystemService(
						KPApplication.getInstance().CONNECTIVITY_SERVICE);
	...
	public static String getInfo(String method, String url, String[] keys,
			String[] values) throws KPException {
		JSONObject obj = null;
		...
		if (str != null) {
			try {
				...
				if (!"1".equals(obj.getString("status")))
					throw new KPException(Integer.parseInt(obj.getString("status")),
	 ...
	 * @throws KPException
	 */
	private static String getOrDelete(String method, String url, String[] keys,
			String[] values) throws KPException {
		if (!checkConnection()) {
			throw new KPException(R.string.net_error_code, R.string.net_error);
		}
		...
		DTLog.i(url);
		// 读取本地缓存，否则网络加载
		String result = KPHttpCache.loadFromLocal(url);
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				...
				throw new KPException(R.string.server_error_code,
						R.string.server_error);
			}
			result = EntityUtils.toString(response.getEntity(), "UTF-8");
			DTLog.i(result);
			KPHttpCache.saveToLocal(result, url);// 数据缓存到本地
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			client.getConnectionManager().shutdown();// 释放连接
	...
	public static String postInfo(String method, String url, String[] keys,
			String[] values) throws KPException {
		JSONObject obj = null;
		String str = post(method, url, keys, values);
		if (str != null) {
			try {
				obj = new JSONObject(str);
				if (!"1".equals(obj.getString("status")))
	 ...
	 * @throws KPException
	 */
	private static String post(String method, String url, String[] keys,
			String[] values) throws KPException {
		if (!checkConnection()) {
			throw new KPException(R.string.net_error_code, R.string.net_error);
		}
	 ...
	 * @throws KPException
	 */
	public static String postUpFile(String url, File file, String[] keys,
			String[] values) throws KPException {
		DTLog.i(url);
		JSONObject obj = null;
		String str = postFile(url, file, keys, values);
		if (str != null) {
			try {
				obj = new JSONObject(str);
				if (obj.getInt("status") != 1)
					throw new KPException(obj.getInt("status"),
							obj.getString("message"));
			} catch (JSONException e) {
				e.printStackTrace();
			}
	 ...
	 * @throws KPException
	 */
	private static String postFile(String url, File file, String[] keys,
			String[] values) throws KPException {
		if (!checkConnection()) {
			throw new KPException(R.string.net_error_code, R.string.net_error);
		}
		...
		try {
			...
			if (keys != null)
				for (int i = 0; i < keys.length; i++) {
					DTLog.i("key: "+keys[i]+"   values: "+values[i]);
					reqEntity.addPart(keys[i], new StringBody(values[i]));
				}
