...
import com.rtmap.experience.core.KPThreadManager;
...
public class DTLoadingPage extends FrameLayout {
	...
	public DTLoadingPage(Context context) {
		super(context);
		init();
	}
	...
	private void init() {
		setBackgroundColor(DTUIUtils.getColor(R.color.white));// 设置背景
	...
	private void showPageSafe() {
		DTUIUtils.runInMainThread(new Runnable() {
			@Override
			public void run() {
				showPage();
			}
	...
	private synchronized void showPage() {
		...
		DTLog.e("mSuccessView : mstate:" + mState + "     mSuccessView:"
				+ mSucceedView);
		if (null != mSucceedView) {
			DTViewUtils.removeSelfFromParent(this);
	...
	public synchronized void show() {
		...
		if (mState == STATE_UNLOADED) {
			...
			KPThreadManager.getLongPool().execute(task);
		}
		showPageSafe();
	}

	protected View createLoadingView() {
		return DTUIUtils.inflate(R.layout.loading_page_loading);
	}

	protected View createEmptyView() {
		return DTUIUtils.inflate(R.layout.loading_page_empty);
	}

	protected View createErrorView() {
		View view = DTUIUtils.inflate(R.layout.loading_page_error);
	...
	class LoadingTask implements Runnable {
		...
		public void run() {
			...
			DTUIUtils.runInMainThread(new Runnable() {// 主线程改变UI
						@Override
						public void run() {
