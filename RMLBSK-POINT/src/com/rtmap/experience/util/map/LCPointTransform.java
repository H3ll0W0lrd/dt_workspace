...
public class LCPointTransform {
	...
	public float xBumpTest(float dx) {
		if (mBitmapEnvelope._minx == 0 && mBitmapEnvelope._maxx < mScreenWidth
				&& dx < 0)
			return 0;
		if (mBitmapEnvelope._maxx == mScreenWidth && mBitmapEnvelope._minx > 0
				&& dx > 0)
			return 0;
		if (mBitmapEnvelope._maxx + dx > mScreenWidth
				&& mBitmapEnvelope._minx + dx > 0) {
		...
		if (mBitmapEnvelope._minx + dx < 0
				&& mBitmapEnvelope._maxx + dx < mScreenWidth) {
			if (mBitmapEnvelope.getWidth() > mScreenWidth)
				return mScreenWidth - mBitmapEnvelope._maxx;
			else
				return -mBitmapEnvelope._minx;
	...
	public float yBumpTest(float dy) {
		if (mBitmapEnvelope._miny == 0 && mBitmapEnvelope._maxy < mScreenHeight
				&& dy < 0)
			return 0;
		if (mBitmapEnvelope._maxy == mScreenHeight && mBitmapEnvelope._miny > 0
				&& dy > 0)
			return 0;
		if (mBitmapEnvelope._maxy + dy > mScreenHeight
				&& mBitmapEnvelope._miny + dy > 0) {
			if (mBitmapEnvelope.getHeight() > mScreenHeight)
				return -mBitmapEnvelope._miny;
			else
				return mScreenHeight - mBitmapEnvelope._maxy;
		}
		if (mBitmapEnvelope._miny + dy < 0
				&& mBitmapEnvelope._maxy + dy < mScreenHeight) {
			if (mBitmapEnvelope.getHeight() > mScreenHeight)
				return mScreenHeight - mBitmapEnvelope._maxy;
			else
				return -mBitmapEnvelope._miny;
	...
	public void setBitmapDimension(int width, int height) {
		if (mBitmapEnvelope.getWidth() == width
				&& mBitmapEnvelope.getHeight() == height)
			return;
		if (width == 0 || height == 0)
			return;
		if (mBitmapEnvelope.isNull()) {
			...
			LCPoint c = new LCPoint();
			screenToBitmap(mScreenWidth / 2, mScreenHeight / 2, c);
			float rx = (float) c.getX() / (float) mBitmapEnvelope.getWidth();
			float ry = (float) c.getY() / (float) mBitmapEnvelope.getHeight();
			int nx = (int) (width * rx);
			int ny = (int) (height * ry);
			int offsetx = nx - c.getX();
			int offsety = ny - c.getY();
	 ...
	 * @return
	 */
	public boolean clientToWorld(float clix, float cliy, LCPoint c) {
		if (mEnvelope == null)
			return false;
		if (c == null)
			return false;
		if (!screenToBitmap(clix, cliy, c))
			return false;
		c.setX( (int) ((mEnvelope._minx + (int) (c.getX() / mRatioX))));
		c.setY((int) ((mEnvelope._miny + (int) (c.getY()/ mRatioY))));
		DTLog.i("scale : "+mScale);
		return true;
	}
	 ...
	 * @param ratio
	 */
	public void zoom(float ratio,float x,float y) {
	 ...
	 * @return
	 */
	private boolean screenToBitmap(float sx, float sy, LCPoint c) {
		c.setX((int) (sx - mBitmapEnvelope._minx));// + (int)(sx / mSrcRatioX);
		c.setY((int) (sy - mBitmapEnvelope._miny));// + (int)(sy / mSrcRatioY);
		return true;
	}
	...
	private void reset() {
		...
		if (mEnvelope.getWidth() == 0 || mEnvelope.getHeight() == 0)
			return;
		mRatioX = ((float) mBitmapEnvelope.getWidth())
				/ (float) (mEnvelope.getWidth());
		mRatioY = ((float) mBitmapEnvelope.getHeight())
				/ (float) (mEnvelope.getHeight());
	}
	...
	private void moveCenter() {
		...
		if (mDirty)
			return;
		mBitmapEnvelope.translate(-mBitmapEnvelope._minx,
				-mBitmapEnvelope._miny);
