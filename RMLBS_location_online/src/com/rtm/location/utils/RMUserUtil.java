...
public class RMUserUtil {
	...
	private static class UserCall implements RMCallBack {
		...
		public UserCall(Context context, String key, OnGetUserListener listener) {
		...
		public Object onCallBackStart(Object... o) {
			try {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO
					...
					obj.put("cpu_type", android.os.Build.SUPPORTED_ABIS);
				obj.put("os_version", Build.VERSION.RELEASE);
				obj.put("android_id", Settings.Secure.getString(
						context.getContentResolver(),
						Settings.Secure.ANDROID_ID));
				String result = RMHttpUtil.postHttpsConnection(
						RMHttpUrl.LBS_USER_ID, obj.toString());
				// {"result":{"req":"lbsid","error_code":"0","error_msg":"Success"},"lbsid":"","delaylocate_time":"0",
				// "isbadlog_return":"1","isphone_whitelist":"1","expiration_time":"1475481783593","log_record_time":"300",
				// "bei_md5":"0d7a7ceffadb830ebdf49658c6da905f","bei_url":"http://lbsdata.rtmap.com/map/beacons.bei"}
				if (result != null && !RMHttpUtil.NET_ERROR.equals(result)) {
					...
					if (Integer.parseInt(errorobj.getString("error_code")) == 0) {
						...
						user.setExpiration_time(resultobj
								.getString("expiration_time"));
						user.setIsbadlog_return(Integer.valueOf(resultobj
								.getString("isbadlog_return")));
						user.setIsphone_whitelist(Integer.valueOf(resultobj
								.getString("isphone_whitelist")));
						user.setBei_md5(resultobj.getString("bei_md5"));
						user.setBei_url(resultobj.getString("bei_url"));
						user.setLog_record_time(Integer.valueOf(resultobj
								.getString("log_record_time")));
