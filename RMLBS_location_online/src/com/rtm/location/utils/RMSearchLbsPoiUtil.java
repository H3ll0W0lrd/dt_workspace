...
public class RMSearchLbsPoiUtil {
	 ...
	 * 
	 * @param key 智慧图LBS开放平台key
	 * @param location 定位点位置
	 * @param listener 搜索结果回调接口
	 */
	...
	private static class CheckCall implements RMCallBack {
		...
		private OnSearchLbsPoiListener onSearchPoiListener;
					...
					if (route.getError_code() == 0) {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							if (pointobj.has("build_name")) {
								point.setBuildName(pointobj.getString("build_name"));
							}
							if (pointobj.has("is_inside")) {
								point.setIsInside(pointobj.getString("is_inside"));
							}
