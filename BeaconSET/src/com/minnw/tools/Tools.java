...
public class Tools {
	...
	public static final UUID PRIVATE_UUID = java.util.UUID
			.fromString("0000fffa-0000-1000-8000-00805f9b34fb");
	...
	static public int encodeProperty(JSONArray ary) {
		int property = 0;
		if (PROPERTY_BROADCAST
				.equals(getDataFromArray(ary, PROPERTY_BROADCAST))) {
			property = property + 1;
		}
		if (PROPERTY_READ.equals(getDataFromArray(ary, PROPERTY_READ))) {
			property = property + 2;
		}
		if (PROPERTY_WRITE_NO_RESPONSE.equals(getDataFromArray(ary,
		...
		if (PROPERTY_EXTENDED_PROPS.equals(getDataFromArray(ary,
	...
	static public int encodePermission(JSONArray ary) {
		...
		if (PERMISSION_READ_ENCRYPTED.equals(getDataFromArray(ary,
				PERMISSION_READ_ENCRYPTED))) {
			permission = permission + 2;
		}
		if (PERMISSION_READ_ENCRYPTED_MITM.equals(getDataFromArray(ary,
		...
		if (PERMISSION_WRITE_ENCRYPTED.equals(getDataFromArray(ary,
				PERMISSION_WRITE_ENCRYPTED))) {
			permission = permission + 32;
		}
		if (PERMISSION_WRITE_ENCRYPTED_MITM.equals(getDataFromArray(ary,
				PERMISSION_WRITE_ENCRYPTED_MITM))) {
			permission = permission + 64;
		}
		if (PERMISSION_WRITE_SIGEND.equals(getDataFromArray(ary,
				PERMISSION_WRITE_SIGEND))) {
			permission = permission + 128;
		}
		if (PERMISSION_WRITE_SIGEND_MITM.equals(getDataFromArray(ary,
				PERMISSION_WRITE_SIGEND_MITM))) {
	...
	public static JSONArray getArray(JSONArray jsonArray, String key) {
		...
		try {
			newJsonArray = new JSONArray(getObjectFromArray(jsonArray).get(key)
					.toString());
		} catch (JSONException e) {
			e.printStackTrace();
		}
	...
	public static JSONArray getArray(JSONArray jsonArray, int objectIndex,
		...
		JSONArray newJsonArray = null;
		try {
			newJsonArray = new JSONArray(getObjectFromArray(jsonArray,
					objectIndex).get(key).toString());
		} catch (JSONException e) {
			e.printStackTrace();
		}
		return newJsonArray;
...
//	}
	...
	public static byte[] hexStringToByte(String hexString) {
		...
		for (int i = 0; i < len; i++) {
			int pos = i * 2;
			result[len - i - 1] = (byte) ("0123456789abcdef"
					.indexOf(achar[pos]) << 4 | "0123456789abcdef"
					.indexOf(achar[pos + 1]));
		}
		return result;
	}
	...
	public static boolean isSupportSpecificAPI(String specificBrand) {
		if (getPhoneBrand().toLowerCase().indexOf(specificBrand.toLowerCase()) != -1
				|| getPhoneManufacturers().toLowerCase().indexOf(
						specificBrand.toLowerCase()) != -1) {
			return true;
		} else {
			return false;
		}
