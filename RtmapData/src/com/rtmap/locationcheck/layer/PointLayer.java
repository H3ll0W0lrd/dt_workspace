...
public class PointLayer implements BaseMapLayer {
	...
	private ArrayList<LCPoint> mPointList;

	@Deprecated
	public PointLayer(MapView view) {
		mMapView = view;
		mPointList = new ArrayList<LCPoint>();
		initLayer(view);
	}
	 ...
	 *            本楼层的POI点集合
	 */
	public boolean addPointList(List<LCPoint> points) {
	 ...
	 *            本楼层的POI点
	 */
	public boolean addPoint(LCPoint point) {
	 ...
	 * @return
	 */
	public LCPoint getPoint(int i) {
		if (i > mPointList.size() - 1)
			return null;
		return mPointList.get(i);
	}

	public ArrayList<LCPoint> getPointList() {
		return mPointList;
	}
	 ...
	 * @param point
	 */
	public void clearPoint(LCPoint point) {
	...
	public void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mPointList.size(); i++) {
			...
			if (mClickPointIcon != null && p.isClick())
				canvas.drawBitmap(mClickPointIcon, pointX, pointY, null);// 画出点
			else
