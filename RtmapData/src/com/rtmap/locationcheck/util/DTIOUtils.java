...
public class DTIOUtils {
	
	private final static int IMAGE_AVATAR_SIZE = 1280;
	...
	public static File rotaingImageView(String fileUrl, Context context) {
		...
		DTLog.i("filePath : "+fileUrl+"     outWidth:" + options.outWidth + "     outHeight:"
				+ options.outHeight);
		if (options.outWidth <= IMAGE_AVATAR_SIZE && options.outHeight <= IMAGE_AVATAR_SIZE) {// 如果宽高都小于规定尺寸
			...
			DTLog.i("options.inSampleSize: " + options.inSampleSize);
		}
		 ...
		 */
		bitmap = ThumbnailUtils.extractThumbnail(bitmap, IMAGE_AVATAR_SIZE, IMAGE_AVATAR_SIZE);// 得到裁剪后的图片
		int degree = readPictureDegree(fileUrl);
		DTLog.i("degree:" + degree);
		// 旋转图片 动作
