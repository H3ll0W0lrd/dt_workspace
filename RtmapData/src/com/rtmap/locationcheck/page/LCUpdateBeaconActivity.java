...
public class LCUpdateBeaconActivity extends LCActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mStatusStr = getResources().getStringArray(
				R.array.beacon_work_new_status);

		mInfo = (BeaconInfo) getIntent().getExtras().getSerializable("beacon");

		if (mInfo != null) {
			int a = Math.abs(mInfo.getWork_status());
			if (a > 1) {
				mWorkStatus.setText(mStatusStr[a - 1]);
			} else {
				mWorkStatus.setText(mStatusStr[a]);
			}
		...
		mInterList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				if (status == position)
					return;
				if (position > 0) {
					mInfo.setWork_status(-position - 1);
				} else {
