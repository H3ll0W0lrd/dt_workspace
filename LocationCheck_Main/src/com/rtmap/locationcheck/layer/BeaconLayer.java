...
public class BeaconLayer implements BaseMapLayer {
	 ...
	 *            中间点标记图标
	 */
	public BeaconLayer(MapView view, HashMap<Integer, Bitmap> statusBitmap) {
		mMapView = view;
		...
		for (int i = 0; i < mPointList.size(); i++) {
			...
			if (isType) {
				if (p.getMac().startsWith("C91A")) {
					canvas.drawBitmap(mIconMap.get(4), pointX, pointY, mBeaconPaint);
				} else {
					canvas.drawBitmap(mIconMap.get(5), pointX, pointY, mBeaconPaint);
				}
				...
				if (p.getWork_status() == -4) {
					if (p.getEdit_status() == 1) {
						canvas.drawBitmap(mIconMap.get(2), pointX, pointY,
								mBeaconPaint);
					} else
						canvas.drawBitmap(mIconMap.get(3), pointX, pointY,
								mBeaconPaint);
				} else if (p.getWork_status() == 0) {
					if (p.getEdit_status() == 0 || p.getEdit_status() == 2) {// 正常
						canvas.drawBitmap(mIconMap.get(0), pointX, pointY,
								mBeaconPaint);
					} else if (p.getEdit_status() == 1) {
						canvas.drawBitmap(mIconMap.get(2), pointX, pointY,
								mBeaconPaint);
					} else {
						canvas.drawBitmap(mIconMap.get(1), pointX, pointY,
								mBeaconPaint);
					}
				} else {
					if (p.getEdit_status() == 1) {
