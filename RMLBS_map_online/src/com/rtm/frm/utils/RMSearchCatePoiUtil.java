...
public class RMSearchCatePoiUtil {
	 ...
	 * @return 本类对象
	 */
	public RMSearchCatePoiUtil setOnSearchCateListener(
			OnSearchCateListener onSearchCateListener) {
	...
	private class CheckCall implements RMCallBack {
				...
				if (result != null && !RMHttpUtil.NET_ERROR.equals(result)) {
					...
					if (route.getError_code() == 0) {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							point.setX(Float.parseFloat(pointobj.getString("x")));
							point.setY(Float.parseFloat(pointobj.getString("y")));
		...
		public void onCallBackFinish(Object obj) {
			if (onSearchCateListener != null)
				onSearchCateListener.onFinished((RMPois) obj);
		}
	}

	/**
	 * 某一建筑物内的分类搜索POI结果回调接口
	 * @author dingtao 
	 */
	public interface OnSearchCateListener {
		/**
		 * POI结果回调方法
		 * @param result 详情请参见RMPois
		 */
