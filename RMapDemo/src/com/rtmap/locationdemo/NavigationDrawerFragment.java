...
public class NavigationDrawerFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		XunluMap.getInstance().init(getActivity());// 初始化
		/**
		 * 建筑物列表工具，可以获取开发者申请的key支持的建筑物，帮助文档可以参照开发者平台地图API使用
		 */
		RMBuildListUtil.requestBuildList(XunluMap.getInstance().getApiKey(),
				null, new RMBuildListUtil.OnGetBuildListListener() {
					...
					public void onFinished(RMBuildList result) {
						if (result.getError_code() == 0) {
							...
							for (int i = 0; i < result.getCitylist().size(); i++) {
								...
								for (int j = 0; j < city.getBuildlist().size(); j++) {
									count++;
									if (count == 6)
										return;
									BuildInfo info = city.getBuildlist().get(j);
									RMBuildDetailUtil
											.requestBuildDetail(
													...
													new RMBuildDetailUtil.OnGetBuildDetailListener() {
														...
														public void onFinished(
																RMBuildDetail result) {
															if (result
																	.getError_code() == 0) {// 当错误码为0时，请获取数据
