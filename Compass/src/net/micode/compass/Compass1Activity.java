...
public class Compass1Activity extends Activity {
	...
	private Sensor aSensor,mSensor;//加速度传感器和磁力传感器

	@Override
	protected void onResume() {// 在恢复的生命周期里判断、启动位置更新服务和传感器服务
		super.onResume();
		mSensorManager.registerListener(mOrientationSensorEventListener,
				aSensor, SensorManager.SENSOR_DELAY_UI);// 注册传感器监听器
	...
	protected void onPause() {// 在暂停的生命周期里注销传感器服务和位置更新服务
		super.onPause();
		mStopDrawing = true;
		mSensorManager.unregisterListener(mOrientationSensorEventListener, aSensor);
		mSensorManager.unregisterListener(mOrientationSensorEventListener, mSensor);
	}
	...
	private void initServices() {
		...
		Log.i("rtmap", "加速度计："+(aSensor==null?"没有":"有"));

		mSensor = mSensorManager
				.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);// 磁力传感器
		Log.i("rtmap", "磁力传感器："+(mSensor==null?"没有":"有"));
	...
	private SensorEventListener mOrientationSensorEventListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			...
			SensorManager.getRotationMatrix(rotate, null, accelerometerValues,
					magneticFieldValues);// 通过磁力和加速度值计算旋转矩阵，赋值给rotate

			SensorManager.getOrientation(rotate, values);// 最后通过矩阵数组计算x,y,z方向手机角度，目前我们需要第一个角度x轴的

			values[0] = (float) Math.toDegrees(values[0]);

//			Log.i("test", "定位角度是：" + values[0]+"   "+values[1]+"   "+values[2]);// 此结果为从北到南顺时针为0-180度，从南-北顺指针-180到0度，其实是弧度运算的，也就是从北顺指针是0-360度
		}
