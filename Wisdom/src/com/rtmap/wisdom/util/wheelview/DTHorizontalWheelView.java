package com.rtmap.wisdom.util.wheelview;
 ...
 */
public class DTHorizontalWheelView extends View implements
		GestureDetector.OnGestureListener {
	public static final float DEFAULT_INTERVAL_FACTOR = 2f;
	public static final float DEFAULT_MARK_RATIO = 0.1f;
...
//	private int mItems.size();
	...
	public DTHorizontalWheelView(Context context) {
		super(context);
		init(null);
	}

	public DTHorizontalWheelView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}
	...
	protected void init(AttributeSet attrs) {
		...
		TypedArray ta = attrs == null ? null : getContext()
				.obtainStyledAttributes(attrs,
						R.styleable.DTHorizontalWheelView);
		if (ta != null) {
			mHighlightColor = ta.getColor(
					R.styleable.DTHorizontalWheelView_lwvHighlightColor,
					mHighlightColor);
			mMarkTextColor = ta.getColor(
					R.styleable.DTHorizontalWheelView_lwvMarkTextColor,
					mMarkTextColor);
			mMarkColor = ta.getColor(
					R.styleable.DTHorizontalWheelView_lwvMarkColor, mMarkColor);
			mIntervalFactor = ta.getFloat(
					R.styleable.DTHorizontalWheelView_lwvIntervalFactor,
					mIntervalFactor);
			mMarkRatio = ta.getFloat(
					R.styleable.DTHorizontalWheelView_lwvMarkRatio, mMarkRatio);
			mAdditionCenterMark = ta
					.getString(R.styleable.DTHorizontalWheelView_lwvAdditionalCenterMark);
			mCenterTextSize = ta.getDimension(
					R.styleable.DTHorizontalWheelView_lwvCenterMarkTextSize,
					mCenterTextSize);
			mNormalTextSize = ta.getDimension(
					R.styleable.DTHorizontalWheelView_lwvMarkTextSize,
					mNormalTextSize);
			mCursorSize = ta.getDimension(
					R.styleable.DTHorizontalWheelView_lwvCursorSize,
					mCursorSize);
		}
	...
	private void calcIntervalDis() {
		...
		if (mItems != null && mItems.size() > 0) {
			...
			mMarkTextPaint.getTextBounds(defaultText, 0, defaultText.length(),
					temp);
			max = temp.width();
		}
		...
		if (!TextUtils.isEmpty(mAdditionCenterMark)) {
			...
			mMarkTextPaint.getTextBounds(mAdditionCenterMark, 0,
					mAdditionCenterMark.length(), temp);
			mAdditionCenterMarkWidth = temp.width();
			max += temp.width();
		}

		mIntervalDis = max * mIntervalFactor;
		DTLog.i("最大值："+max+"   "+mIntervalDis);
	}
	...
	public void fling(int velocityX, int velocityY) {
		mScroller.fling(getScrollX(), getScrollY(), velocityX, velocityY,
				...
				(int) (mContentRectF.width() - mMaxOverScrollDistance), 0, 0,
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w != oldw || h != oldh) {
			...
			DTLog.i("最大距离为："+mMaxOverScrollDistance+"    "+mIntervalDis);
			mContentRectF.set(0, 0, (mItems.size() - 1) * mIntervalDis, h);
	...
	protected void onDraw(Canvas canvas) {
		...
		// mCenterIndicatorPath.moveTo(mMaxOverScrollDistance - sizeDiv2 +
		// getScrollX(), 0);
		...
		end = Math.min(end, mItems.size() + mViewScopeSize * 2);
		...
		smallMarkShrinkY = Math.min((markHeight - mMarkWidth) / 2f,
				smallMarkShrinkY);
		...
		for (int i = start; i < end; i++) {
			...
			//
			// if (i >= 0 && i <= mItems.size() && mCenterIndex == i) {
			...
			// // center mark
			// mMarkPaint.setStrokeWidth(mCenterMarkWidth);
			// canvas.drawLine(ox, mTopSpace, ox, mTopSpace + markHeight,
			// mMarkPaint);
			// } else {
			// // other small mark
			// mMarkPaint.setStrokeWidth(mMarkWidth);
			// canvas.drawLine(ox, mTopSpace + smallMarkShrinkY, ox, mTopSpace +
			// markHeight - smallMarkShrinkY, mMarkPaint);
			// }
			// }

			// mark text
			if (mItems.size() > 0 && i >= 0 && i < mItems.size()) {
				...
				if (mCenterIndex == i) {
					...
					if (!TextUtils.isEmpty(mAdditionCenterMark)) {
						...
						float tsize = mMarkTextPaint.measureText(temp, 0,
								temp.length());
						// canvas.drawText(temp, 0, temp.length(), x - off,
						// mHeight - mBottomSpace, mMarkTextPaint);
						canvas.drawText(temp, 0, temp.length(), x - off,
								mHeight / 2 + mNormalTextSize / 2,
								mMarkTextPaint);
						mMarkTextPaint.setTextSize(mNormalTextSize);
						// canvas.drawText(mAdditionCenterMark, x + tsize / 2f,
						// mHeight - mBottomSpace, mMarkTextPaint);
						canvas.drawText(mAdditionCenterMark, x + tsize / 2f,
								mHeight / 2 + mNormalTextSize / 2,
								mMarkTextPaint);
					} else {
						// canvas.drawText(temp, 0, temp.length(), x, mHeight -
						// mBottomSpace, mMarkTextPaint);
						canvas.drawText(temp, 0, temp.length(), x, mHeight / 2
								+ mNormalTextSize / 2, mMarkTextPaint);
					}
				} else {
		...
		if (!mFling && MotionEvent.ACTION_UP == event.getAction()) {
			if (getScrollX() < -mMaxOverScrollDistance) {
				mScroller.startScroll(getScrollX(), 0,
						(int) -mMaxOverScrollDistance - getScrollX(), 0);
				invalidate();
				ret = true;
			} else if (getScrollX() > mContentRectF.width()
					- mMaxOverScrollDistance) {
				
				mScroller.startScroll(getScrollX(), 0,
						(int) (mContentRectF.width() - mMaxOverScrollDistance)
								- getScrollX(), 0);
				invalidate();
				ret = true;
			} else {
	...
	private void refreshCenter(int offsetX) {
		...
		if (mCenterIndex < 0) {
			mCenterIndex = 0;
		} else if (mCenterIndex > mItems.size() - 1) {
			mCenterIndex = mItems.size() - 1;
		}
	...
	public void selectIndex(int index) {
		...
		post(new Runnable() {
			...
			public void run() {
				scrollTo(
						(int) (mCenterIndex * mIntervalDis - mMaxOverScrollDistance),
						0);
				invalidate();
				refreshCenter();
			}
...
//		mItems.size() = null == mItems ? 0 : mItems.size();
		mCenterIndex = Math.min(0, mItems.size());
		calcIntervalDis();
		mContentRectF.set(0, 0, (mItems.size() - 1) * mIntervalDis, mHeight);
		mViewScopeSize = (int) Math.ceil(mMaxOverScrollDistance
				/ mIntervalDis);
		invalidate();
	}
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		if (scrollX < -mMaxOverScrollDistance
				|| scrollX > mContentRectF.width() - mMaxOverScrollDistance) {
			return false;
		} else {
			mFling = true;
