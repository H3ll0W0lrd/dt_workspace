...
public class TapPOILayer implements BaseMapLayer {
	 ...
	 * 
	 * @param listener
	 *            OnTapPOIDrawListener监听POI点击，绘制View
	 */
	...
	public boolean onTap(MotionEvent e) {
		...
		if (actionCode == MotionEvent.ACTION_UP) {
			...
			long time = System.currentTimeMillis();
			Shape[] shapes = mMapView.getConfig().getDrawMap().getLayer().shapes;
			...
			if (mOnMapTapedListener != null && time - pressStartTime <= 800) {
				...
				if (clickShape != null) {
					POI poi = new POI(clickShape.mId, clickShape.mName,
							mMapView.getConfig().getBuildId(), mMapView
									.getConfig().getFloor(),
							clickShape.mCenter.mX / 1000f,
							clickShape.mCenter.mY / 1000f);
					poi.setType(clickShape.mType);
					mOnMapTapedListener.onMapTaped(poi,
							mMapView.fromPixels(e.getX(), e.getY()));
				} else {
					mOnMapTapedListener.onMapTaped(null,
							mMapView.fromPixels(e.getX(), e.getY()));
				}
			}
			// mMapView.deleteAllView();
