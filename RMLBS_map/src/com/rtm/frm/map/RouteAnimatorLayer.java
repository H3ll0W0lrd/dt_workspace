...
public class RouteAnimatorLayer implements BaseMapLayer,
	...
	public void onDraw(Canvas canvas) {
		...
		while (keySet.hasNext()) {
			...
			if (mPointIcon == null) {
				for (int i = 0; i < points.size(); i++) {
					...
					PointInfo temppoi = mMapView.fromLocation(new Location(p
							.getX(), p.getY()));
					if (p.getBuildId().equals(mMapView.getBuildId())
							&& p.getFloor().equals(mMapView.getFloor())) {
						if (mPath.isEmpty()) {
							mPath.lineTo(temppoi.getX(), temppoi.getY());
						} else {
							mPath.moveTo(temppoi.getX(), temppoi.getY());
						}
					} else {
						if(!mPath.isEmpty()){
			...
			if (isSameLocation(mImagePoint) && mNavigationIcon != null
					&& isStartAnim) {// 导航图标
				PointInfo temppoi1 = mMapView.fromLocation(new Location(
						mImagePoint.getX(), mImagePoint.getY()));
