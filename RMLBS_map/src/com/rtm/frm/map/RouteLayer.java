...
public class RouteLayer implements BaseMapLayer {
	...
	public static int DISTANCE_END = 10;// 结束导航的距离
	...
	public void updateLocation(Location location) {
		...
		if (mNavigateRoutePointList.size() > 1 && !isLock) {
			...
			if (mLimitCount > COUNT_LIMIT && !isLock && isNavigating) {// 重新规划路线:超过次数并且图层没有锁定并且开启导航
		...
		if (mNavigateRoutePointList.size() < 3
				&& mNavigateRoutePointList.size() > 0 && !isLock) {
			// 进行距离判断然后到达终点了
