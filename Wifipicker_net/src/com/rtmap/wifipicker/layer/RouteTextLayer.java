...
public class RouteTextLayer implements BaseMapLayer {
	...
	private Bitmap mRouteIcon;// 中间点icon
				...
				for (int i = 0; i < points.size(); i++) {
					...
					if (i != 0) {
						...
						if (i > 0 && i < points.size() - 1)
							canvas.drawBitmap(mRouteIcon, pointX, pointY, null);
						else
							canvas.drawBitmap(mRouteIcon, pointX, pointY, null);
					} else {
						canvas.drawBitmap(mRouteIcon, pointX, pointY, null);
	...
	private void drawText(Canvas canvas, RMPoi p) {
		if (p == null)
			return;
		PointInfo temppoi = mMapView.fromLocation(new Location(p.getX(), p.getY()));
		...
		if (e.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (Math.abs(e.getX() - downX) < 20
				...
				if (p2p > -1 && mClickListener != null) {// 说明点击在点的范围内
					DTLog.e("点击事件clickPointX : "+clickPoint.getX()+"     Y : "+clickPoint.getY());
